...
// Get user settings (onboarding state, shop name, avatar)
app.get("/user/settings", verifyToken, (req, res) => {
  ...
  if (!row) {
    db.prepare(
      "INSERT INTO user_settings (uid, shop_name, avatar_index, onboarded) VALUES (?, 'My Shop', -1, 0)"
    ).run(uid);
...
app.put("/user/settings", verifyToken, (req, res) => {
  const uid = req.user.uid;
  const { shopName, avatarIndex, onboarded } = req.body;

  const existing = db.prepare("SELECT uid FROM user_settings WHERE uid = ?").get(uid);
  if (!existing) {
    db.prepare(
      "INSERT INTO user_settings (uid, shop_name, avatar_index, onboarded) VALUES (?, ?, ?, ?)"
    ).run(uid, shopName || "My Shop", avatarIndex ?? -1, onboarded ? 1 : 0);
  } else {
    const updates = [];
    const params = [];
